...
def _paths():
    cred=Path(os.getenv("GMAIL_CREDENTIALS_FILE","config/google_client_secret.json"))
    token=Path(os.getenv("GMAIL_TOKEN_FILE","data/gmail/token.json"))
...
def import_attachments(destination,query=None,max_messages=50):
    svc=service();dest=Path(destination);dest.mkdir(parents=True,exist_ok=True)
    query=query or os.getenv("GMAIL_QUERY","has:attachment newer_than:30d")
    ...
    for item in res.get("messages",[]):
        ...
        while stack:
            ...
            if not data and body.get("attachmentId"):
                data=svc.users().messages().attachments().get(userId="me",messageId=item["id"],id=body["attachmentId"]).execute().get("data")
            if data:
                path=dest/Path(fn).name
                path.write_bytes(base64.urlsafe_b64decode(data.encode()))
